...
struct DBUser: Codable {
    ...
    init(auth: AuthDataResultModel) {
        ...
        self.photoUrl = auth.photoURL?.description
...
final class UserManager {
    ...
    private let userCollection: CollectionReference = Firestore.firestore().collection("user")
    ...
    private let decoder: Firestore.Decoder = {
        ...
        //        decoder.keyDecodingStrategy = .convertFromSnakeCase ------- we no longer need as we have the enum coding keys already
        return decoder
    }()
    ...
    //        try userDocument(userId: user.userId).setData(from: user, merge: true) // This is dangerous, you're overwriting the whole user profile, not just one parameter
    //    }
    ...
    func removeUserFavoriteProduct(userId: String, favoriteProductId: String) async throws {
        try await userFavoriteProductDocument(
            userId: userId,
            favoriteProductId: favoriteProductId).delete()
    }
    ...
    // if you had a chat when they leave the chat we can cancel the listener to avoid executing in the background.
